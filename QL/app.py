...
def generate_question(user_id):
    ...
    try:
        messages_df = pd.read_csv(io.BytesIO(
            supabase.storage.from_(config['supabase']['storage']['bucket'])
            .download(f"{config['supabase']['storage']['paths']['messagePath']}/message.csv")
        ))
    except Exception as e:
